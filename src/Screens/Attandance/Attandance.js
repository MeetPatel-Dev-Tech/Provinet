...
export default function Attendance({ navigation }) {
    ...
    const DailyAttendanceWithBreakTime = ({ item }) => {
        return (
            <View style={{
                ...
                <TouchableOpacity onPress={() => navigation.navigate('DayAllPunchInOutDetail')}>
                    <View>
                        <Text style={{ textAlign: 'center', color: 'black', marginBottom: 10, fontWeight: 'bold', fontSize: 16 }}>{DateChange(item.date)}</Text>
                        <Text style={{ fontSize: 14, color: 'black' }}>InTime : <Text style={{ color: 'gray' }}>{item.intime}</Text> </Text>
                        <Text style={{ fontSize: 14, color: 'black' }}>Outime : <Text style={{ color: 'gray' }}>{item.outtime}</Text> </Text>
                        <Text style={{ fontSize: 14, color: 'black' }}>BreakTime : <Text style={{ color: 'gray' }}>{decimalToHours(item.breakTime)}</Text> </Text>
                        <Text style={{ fontSize: 14, color: 'black' }}>Hours : <Text style={{ color: 'gray' }}>{decimalToHours(item.hours)}</Text> </Text>
                    </View>
                </TouchableOpacity>
            </View>
        )
    ...
    const getEmployePunchInOutDetails = async () => {
        ...
        if (ResponseData.status == 'success') {
            ...
            setPunchOutTime(ResponseData.data[ResponseData.data.length - 1].outtime == null ? null
                : moment(ResponseData.data[ResponseData.data.length - 1].outtime, 'hh:mm').format('hh:mm a'));
            ...
            {!loading &&
                <>
                    <View style={{ marginHorizontal: 20 }}>
                        <View>
                            <Text style={{ color: 'black', fontWeight: 'bold', fontSize: 20, marginTop: 5 }}>PUNCH IN OUT</Text>
                        </View>
                        <View style={{
                    ...
                    <Text style={{ marginTop: 10, fontWeight: 'bold', marginLeft: 20, fontSize: 20, color: 'black' }}>Details</Text>
