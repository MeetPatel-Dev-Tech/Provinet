...
import { View, Text, SafeAreaView, TextInput, Keyboard, FlatList, PermissionsAndroid, StatusBar, TouchableOpacity, Dimensions, Platform, animated, Image, SliderBase, Alert, KeyboardAvoidingView, ImageBackground, Linking } from "react-native"
...
export default function Chat({ route, navigation }) {
    ...
    const Online_Offline = () => {
        ...
        socket.once('chat-list-response', response => {
            console.log('<<>>>chat-list-response', response.chatList[0].online)
            if (response.chatList[0].online == 'Y') {
                setStatus('online')
            }
        })
    }
    ...
    const typingOn = () => {
        ...
        socket.once('chat-list-response', response => {
            ...
            let send = ({
                socket_id: response.chatList[0].socketid,
                typing: 'true',
    ...
    const onClickViewDocument = (item) => {
        ...
        if (isDocFile == false) {
            const url = 'http://45.80.152.23:4553/uploads/socketupload/' + item.filepath; //Constant.KBaseDownloadURL + item.name
    ...
    const findSocketId = () => {
        ...
        socket.once('chat-list-response', response => {
            ...
            // socket.emit('getuserlist', route.params.userId)
    ...
    const renderChatList = ({ item }) => {
        return (
            <View style={{
                ...
                {item.message != null &&
                    <View View style={{
                        backgroundColor: (item.fromuserid || item.fromUserId) != CommonUtilsObj.EmployeDetails[0].user ? '#E9ECF1' : Constant.darkturquoise,
                ...
                {item.message != null &&
                    <View style={{ alignItems: (item.fromuserid || item.fromUserId) == CommonUtilsObj.EmployeDetails[0].user ? 'flex-end' : 'flex-start' }}>
                        <Text style={{ marginTop: 2, fontSize: 10 }}>{moment(item.time, 'HH:mm').format("hh:mm A")}</Text>
                    </View>
                }
                                    ...
                                    <View
                                        style={[
                                            CustomStyle.outgoingMsgBallonContainer,
                                            {
                                                ...
                                                borderBottomLeftRadius: (item.fromuserid || item.fromUserId) == CommonUtilsObj.EmployeDetails[0].user ? 20 : 0,
                                                borderBottomRightRadius: (item.fromuserid || item.fromUserId) == CommonUtilsObj.EmployeDetails[0].user ? 0 : 20,
                                            },
                                        ]}>
                                    ...
                                    <View style={{
                                        ...
                                        <Image source={
                                            item.filePath
                                                ...
                                                : {
                                                    uri:
                                                        'http://45.80.152.23:4553/uploads/socketupload/' +
                                                        item.filepath,
                                                }
                                        }
                            ...
                            {/* {
                                ...
                                <View style={{ width: 200, }}>
                                    <Image
                                        ...
                                        source={require('../../Assets/Image/music.jpg')}
                                        resizeMode="stretch"
                                    />
                                    <Text numberOfLines={2}
                                        style={[
            ...
            >
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <TextInput
                            value={chat}
                            onChangeText={e => { onTypingStart(e), setChat(e) }}
                            style={{
                                flex: 1,
                                padding: 5,
                                paddingHorizontal: 15,
                    ...
                    <View style={{ marginRight: 20, marginLeft: 5 }}>
                        <TouchableOpacity onPress={() => { (chat != '' && sendmessage(false)), typingOff(); }}>
                            <FontAwesome
                                name="paper-plane"
                                size={30}
